...
class GameUI {
    ...
    moveCallToEventiInCorso(call) {
        ...
        if (call.mezziAssegnati && call.mezziAssegnati.length > 0 && window.game && window.game.mezzi) {
            const mezzi = window.game.mezzi.filter(m => (call.mezziAssegnati||[]).includes(m.nome_radio));
            // Filtra solo MSB, MSAB, MSA, ASA, ELI
            const eligibleMezzi = mezzi.filter(m => {
                const tipo = m.tipo_mezzo || '';
                return tipo === 'MSB' || tipo === 'MSAB' || tipo === 'MSA' || tipo === 'ASA' || tipo.includes('ELI');
            });
    ...
    updateMissioneInCorso(call) {
        ...
        if(dettagli) {
            ...
            mezzi.forEach(m => {
                let testoScheda = '';
                const avanzati = mezzi.filter(x => (x.tipo_mezzo && (x.tipo_mezzo === 'MSAB' || x.tipo_mezzo === 'MSA' || x.tipo_mezzo === 'ASA' || (x.tipo_mezzo.toUpperCase().includes('ELI')))) && (x.comunicazioni||[]).some(c => c.toLowerCase().includes('report pronto')));
                ...
                if (hasReportPronto && mostraReport) {
                    // Determina il tipo di mezzo per la ricerca del report (mappatura per Abruzzo)
                    let tipo = '';
                    if (m.tipo_mezzo && m.tipo_mezzo.startsWith('MSB')) tipo = 'MSB';
                    else if (m.tipo_mezzo && m.tipo_mezzo === 'MSAB') tipo = 'MSA1'; // MSAB usa report MSA1
                    else if (m.tipo_mezzo && m.tipo_mezzo === 'MSA') tipo = 'MSA2'; // MSA usa report MSA2
                    else if (m.tipo_mezzo && m.tipo_mezzo === 'ASA') tipo = 'MSA2'; // ASA usa report MSA2
                ...
                if (showMenu) {
                    ...
                    // Per veicoli MSA1, MSA2 e ASA mostriamo solo rientro o accompagna
                    if (m.tipo_mezzo === 'MSA1' || m.tipo_mezzo === 'MSA2' || m.tipo_mezzo === 'ASA') {
            ...
            setTimeout(() => {
                dettagli.querySelectorAll('.btn-conferma-trasporto').forEach(btn => {
                    btn.onclick = () => {
                        ...
                        if ((mezzo.tipo_mezzo === 'MSA1' || mezzo.tipo_mezzo === 'MSA2' || mezzo.tipo_mezzo === 'ASA') && ospedaleSel === '__accompagna__') {
