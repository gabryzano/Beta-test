...
class GameUI {
    ...
    moveCallToEventiInCorso(call) {
        ...
        if (call.mezziAssegnati && call.mezziAssegnati.length > 0 && window.game && window.game.mezzi) {
            const mezzi = window.game.mezzi.filter(m => (call.mezziAssegnati||[]).includes(m.nome_radio));
            // Mostra la destinazione per qualsiasi mezzo che ha ospedale/codice confermato
    ...
    updateMissioneInCorso(call) {
        ...
        if(dettagli) {
            ...
            mezzi.forEach(m => {
                let testoScheda = '';
                // Nuova logica: mostra report ASI sotto ASI, report ASM sotto AM/ASM
                // ASM: tipo_mezzo che inizia per ASM o AM
                // ASI: tipo_mezzo che inizia per ASI
                // ELI: come prima (MSA2)
                const avanzati = mezzi.filter(x => (x.tipo_mezzo && (x.tipo_mezzo.toUpperCase().startsWith('ASM') || x.tipo_mezzo.toUpperCase().startsWith('AM') || x.tipo_mezzo.toUpperCase().startsWith('ASI') || x.tipo_mezzo.toUpperCase().includes('ELI'))) && (x.comunicazioni||[]).some(c => c.toLowerCase().includes('report pronto')));
                ...
                if (hasReportPronto && mostraReport) {
                    let tipo = '';
                    if (m.tipo_mezzo && m.tipo_mezzo.toUpperCase().startsWith('ASI')) tipo = 'ASI';
                    else if (m.tipo_mezzo && (m.tipo_mezzo.toUpperCase().startsWith('ASM') || m.tipo_mezzo.toUpperCase().startsWith('AM'))) tipo = 'ASM';
                    ...
                    if (tipo && call.selectedChiamata) {
                        ...
                        if (call.selectedCase && call.selectedChiamata[call.selectedCase] && call.selectedChiamata[call.selectedCase][tipo]) {
                        ...
                        // Solo se non trova niente in caso_stabile usa un messaggio generico
                            ...
                            if (tipo === 'ASI') {
                                testoScheda = `Paziente soccorso, valutazione clinica ASI. Codice ${codiceColore}. Nessun report dettagliato disponibile.`;
                            } else if (tipo === 'ASM') {
                                testoScheda = `Paziente soccorso, valutazione clinica ASM/AM. Codice ${codiceColore}. Nessun report dettagliato disponibile.`;
                            } else if (tipo === 'MSA2') {
                                testoScheda = `Paziente soccorso, valutazione clinica ELI. Codice ${codiceColore}. Nessun report dettagliato disponibile.`;
                            } else {
                ...
                if (showMenu) {
                    ...
                    if (m.tipo_mezzo === 'MSA1' || m.tipo_mezzo === 'MSA2' || m.tipo_mezzo.toUpperCase().startsWith('AM')) {
                        ...
                        // Define specialist hospitals that should only appear when their specialty is requested
                        const specialistHospitals = {
                            'H BAMBINO GESU': ['PEDIATRIA'],
                            'H BAMBINO GESU PALIDORO': ['PEDIATRIA'],
                            'H CTO': ['OTT'],
                            'H OFTALMICO': ['OFTALMOLOGIA']
                        };
                        
                        // Filter hospitals: exclude specialist hospitals unless their specialty is requested
                        const filteredHospitals = ospedaliOrdinati.filter(o => {
                            const hospitalName = o.nome.trim();
                            if (specialistHospitals[hospitalName]) {
                                // Only include specialist hospital if its specialty is requested
                                return recommendedDept && specialistHospitals[hospitalName].includes(recommendedDept.toUpperCase());
                            }
                            // Include all non-specialist hospitals
                            return true;
                        });
                        ...
                        const selectOsp = `<select class='select-ospedale' data-nome='${m.nome_radio}'>`+
                            ...
                            filteredHospitals.map(o => {
                                ...
                                if (recommendedDept) {
                                    ...
                                    if (specialistHospitals[hospitalName]) {
                                        // For specialist hospitals, only recommend if the required specialty matches exactly
                                        matchRecommended = specialistHospitals[hospitalName].includes(dept);
                                    } else {
                                        // For general hospitals, use the existing logic
    ...
    updateStatoMezzi(mezzoCambiato = null) {
        ...
        const mezzi = allMezzi.filter(m => {
            ...
            // Vehicles of selected central (case-insensitive)
            if ((m.central || '').trim().toLowerCase() === currentCentral) return true;
        ...
        // Raggruppa ospedali per centrale (CORES Nord, Metropolitana, Sud) e ordina per affollamento decrescente
        const centraliTutte = ['CORES Nord', 'CORES Metropolitana', 'CORES Sud'];
        const selCentral = (window.selectedCentral || '').trim();
        // Porta la centrale selezionata in testa
        const centrali = [selCentral, ...centraliTutte.filter(c => c !== selCentral)];
        centrali.forEach(central => {
            const ospedaliCentral = ospedaliOrdinati.filter(h => (h.central || '').trim() === central);
            ospedaliCentral.sort((a, b) => b.pct - a.pct);
            ospedaliCentral.forEach(h => {
                const capacity = Number(h.raw["N° pazienti Max"] || 0);
                const count = (window.game.hospitalPatientCount && window.game.hospitalPatientCount[h.nome]) || 0;
                const pct = capacity ? Math.round((count / capacity) * 100) : 0;
                let extraStyle = '';
                let displayName = h.nome;
                let hospitalCentral = h.central || '';
                if (hospitalCentral && hospitalCentral !== selCentral) {
                    extraStyle = 'background-color:white;';
                    let abbrev = hospitalCentral.replace('CORES ','').toUpperCase();
                    if (abbrev === 'METROPOLITANA') abbrev = 'METRO';
                    displayName = `(${abbrev}) ${h.nome}`;
                }
        ...
        rows.forEach(row => {
            ...
            if (cellComm) {
                cellComm.addEventListener('click', e => {
                    e.stopPropagation();
                    // Marca il report come letto SOLO se si clicca su un report pronto
