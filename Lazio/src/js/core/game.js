...
class EmergencyDispatchGame {
    constructor() {
        ...
        this.missionCounter = { Nord: 0, Metropolitana: 0, Sud: 0 };
    ...
    async loadChiamate() {
        // Carica SEMPRE il file unico delle chiamate, indipendentemente dalla centrale selezionata
        const file = 'src/data/chiamate.json';
        try {
    ...
    async initialize() {
         try {
             ...
             // Aggiorna subito la UI dei mezzi appena caricati
            ...
            // Populate UI with initial vehicle and hospital lists on startup (ospedali)
            if (this.ui && typeof this.ui.updateStatoMezzi === 'function') {
                // Non ordinare qui! L'ordinamento va fatto solo in fase di rendering in UI.js
                this.ui.updateStatoMezzi();
            }
    ...
    initializeMap() {
        ...
        let initCenter = [41.9311, 12.4547]; // default centro Italia
        const initZoom = 13;
        switch (window.selectedCentral) {
            case 'CORES Nord': // Rieti
                initCenter = [42.4048, 12.8570]; break;
            case 'CORES Metropolitana': // Roma, Ospedale San Camillo
                initCenter = [41.8687, 12.4451]; break;
            case 'CORES Sud': // Latina
                initCenter = [41.4676, 12.9037]; break;
        }
    ...
    async loadMezzi() {
        // Carica i mezzi di tutte le centrali (Nord, Metropolitana, Sud)
        const files = [
            { file: 'src/data/mezzi_vt.json', central: 'CORES Nord' },
            { file: 'src/data/mezzi_rm.json', central: 'CORES Metropolitana' },
            { file: 'src/data/mezzi_lt.json', central: 'CORES Sud' }
        ];
        let allMezzi = [];
        try {
            for (const entry of files) {
                ...
                const mezzi = data.map(m => {
                    let lat = m.lat ?? null, lon = m.lon ?? null;
                    if ((!lat || !lon) && m['Coordinate Postazione']) {
                        const coords = String(m['Coordinate Postazione']).split(',').map(s => Number(s.trim())); lat = coords[0]; lon = coords[1];
                    }
                    // Safe string conversion for all fields
                    const nome_radio = (m.nome_radio !== undefined) ? String(m.nome_radio) : (m['Nome radio'] !== undefined ? String(m['Nome radio']) : '');
                    const postazione = (m.postazione !== undefined) ? String(m.postazione) : (m['Nome Postazione'] !== undefined ? String(m['Nome Postazione']) : '');
                    const tipo_mezzo = (m.tipo_mezzo !== undefined) ? String(m.tipo_mezzo) : (m.Mezzo !== undefined ? String(m.Mezzo) : '');
                    const convenzione = (m.convenzione !== undefined) ? String(m.convenzione) : (m['Convenzione'] !== undefined ? String(m['Convenzione']) : '');
                    const Giorni = (m.Giorni !== undefined) ? String(m.Giorni) : (m.giorni !== undefined ? String(m.giorni) : 'LUN-DOM');
                    const orarioLavoro = (m['Orario di lavoro'] !== undefined) ? String(m['Orario di lavoro']) : '';
                    return {
                        nome_radio: nome_radio.trim(),
                        postazione: postazione.trim(),
                        tipo_mezzo: tipo_mezzo,
                        convenzione: convenzione,
                        Giorni: Giorni,
                        'Orario di lavoro': orarioLavoro,
                        lat, lon,
                        stato: 1,
                        _marker: null,
                        _callMarker: null,
            ...
            console.error('Errore fetch mezzi:', e);
            this.mezzi = [];
            this.postazioniMap = {};
        }
    }
    ...
    async loadHospitals() {
        try {
            // Carica ospedali di tutte le centrali
            const files = [
                { file: 'src/data/ps_vt.json', central: 'CORES Nord' },
                { file: 'src/data/ps_rm.json', central: 'CORES Metropolitana' },
                { file: 'src/data/ps_lt.json', central: 'CORES Sud' }
            ];
            let allHospitals = [];
            for (const entry of files) {
                ...
                hospitals = Array.isArray(hospitals) ? hospitals : Object.values(hospitals).find(v => Array.isArray(v)) || [];
                const mapped = hospitals.map(hosp => {
                    ...
                    return {
                        nome: hosp.OSPEDALE?.trim() || hosp.nome?.trim() || "",
                        lat,
                        lon,
                        indirizzo: hosp.INDIRIZZO || hosp.indirizzo || "",
                        raw: hosp,
                        central: entry.central
                    };
                }).filter(h => h.lat !== null && h.lon !== null && !isNaN(h.lat) && !isNaN(h.lon));
                allHospitals = allHospitals.concat(mapped);
            }
            // Ordina: prima quelli della centrale corrente, poi gli altri (ognuno per affollamento decrescente se serve)
            const currentCentral = (window.selectedCentral || '').trim();
            this.hospitals = allHospitals.slice().sort((a, b) => {
                // Prima quelli della centrale corrente
                if (a.central === currentCentral && b.central !== currentCentral) return -1;
                if (a.central !== currentCentral && b.central === currentCentral) return 1;
                // Se stessa centrale, mantieni l'ordine originale (o ordina per affollamento se richiesto)
                return 0;
            });
    ...
    getPostazioneIcon(hasLiberi, isCreli = false) {
        // Sfondo bianco per postazioni esterne rispetto alla centrale selezionata
        const currentCentral = (window.selectedCentral || '').trim();
        let whiteBg = false;
        if (typeof isCreli === 'boolean' && isCreli) {
            whiteBg = true;
        } else if (arguments.length > 2) {
            // compatibilità futura
            whiteBg = !!arguments[2];
        }
        // Determina se la postazione è "esterna" rispetto alla centrale selezionata
        if (this && this.mezzi && this.mezzi.length > 0 && arguments.length === 2) {
            // Trova la central della postazione (primo mezzo associato)
            const postazioneNome = arguments[1];
            const mezziPost = this.mezzi.filter(m => m.postazione === postazioneNome);
            if (mezziPost.length > 0) {
                const central = mezziPost[0].central;
                if (central && central !== currentCentral) whiteBg = true;
            }
        }
    ...
    updatePostazioneMarkers() {
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
            // Trova la central della postazione (primo mezzo associato)
            const mezziPost = this.mezzi.filter(m => m.postazione === postazione.nome);
            let postazioneCentral = mezziPost.length > 0 ? mezziPost[0].central : '';
            const isExternal = postazioneCentral && postazioneCentral !== currentCentral;
            const mezziLiberi = mezziPost.filter(m => m.stato === 1);
            const hasLiberi = mezziLiberi.length > 0;
            // Mostra SOLO i mezzi in stato 1 nel tooltip
            let mezziHtml = '';
            if (mezziLiberi.length > 0) {
                mezziHtml = mezziLiberi.map(m => {
                    let displayName = m.nome_radio;
                    if (m.central && m.central !== currentCentral) {
                        let abbrev = m.central.replace('CORES ','').toUpperCase();
                        if (abbrev === 'METROPOLITANA') abbrev = 'METRO';
                        displayName = `(${abbrev}) ${m.nome_radio}`;
                    }
                    return `<div data-mezzo-id="${m.nome_radio}"><b>${displayName}</b> <span style="color:#555;">(${m.tipo_mezzo || '-'}<\/span> <span style="color:#888;">${m.convenzione ? m.convenzione : ''}</span>)<\/div>`;
                }).join('');
            } else {
                mezziHtml = `<div style="color:#d32f2f;"><i>Nessun mezzo<\/i><\/div>`;
            }
            // Sfondo bianco per postazioni esterne
            const marker = L.marker([postazione.lat, postazione.lon], {
                icon: this.getPostazioneIcon(hasLiberi, postazione.nome, isExternal)
            }).addTo(this.map)
            .bindPopup(`<div style="font-weight:bold;font-size:15px;">${postazione.nome}</div>${mezziHtml}`);
            marker.on('popupopen', () => {
                const mezziPostNow = this.mezzi.filter(m => m.postazione === postazione.nome && m.stato === 1);
                let mezziHtmlNow = '';
                if (mezziPostNow.length > 0) {
                    mezziHtmlNow = mezziPostNow.map(m => {
                        let displayName = m.nome_radio;
                        if (m.central && m.central !== currentCentral) {
                            let abbrev = m.central.replace('CORES ','').toUpperCase();
                            if (abbrev === 'METROPOLITANA') abbrev = 'METRO';
                ...
                marker.setIcon(this.getPostazioneIcon(hasLiberi, postazione.nome, isExternal));
            });
    ...
    updateMezzoMarkers() {
        ...
        this.mezzi.forEach(m => {
           ...
           && m.tipo_mezzo !== 'MSA1_A' && m.tipo_mezzo !== 'MSA2_A') {
            ...
            } else if (m.tipo_mezzo && m.tipo_mezzo.startsWith('AM')) {
                iconUrl = 'src/assets/MSA.png';
            }
    ...
    openMissionPopup(call) {
        ...
        this.updateMissionPopupTable = function(call) {
            ...
            mezziFiltrati.forEach(m => {
                ...
                // Uniforma la visualizzazione del prefisso centrale per mezzi esterni
                const currentCentral = (window.selectedCentral||'').trim();
                let displayName = m.nome_radio;
                if (m.central && m.central !== currentCentral) {
                    let abbrev = m.central.replace('CORES ','').toUpperCase();
        ...
        function attachBtnListeners() {
            document.querySelectorAll('.btn-rapido-mezzo').forEach(btn => {
                btn.onclick = function() {
                    ...
                    let mezziTipo = [];
                    if (tipo === 'ASI') {
                        mezziTipo = mezziFiltrati.filter(m => m.tipo_mezzo && m.tipo_mezzo.toUpperCase().startsWith('ASI') && [1,7].includes(m.stato));
                    } else if (tipo === 'AM/ASM') {
                        mezziTipo = mezziFiltrati.filter(m => m.tipo_mezzo && (m.tipo_mezzo.toUpperCase().startsWith('AM') || m.tipo_mezzo.toUpperCase().startsWith('ASM')) && [1,7].includes(m.stato));
                    } else if (tipo === 'ELI') {
