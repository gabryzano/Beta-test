...
class EmergencyDispatchGame {
    ...
    initializeMap() {
        ...
        let initCenter = [40.1209, 9.0129]; // default Cagliari (Sardegna Sud)
        const initZoom = 13;
        switch (window.selectedCentral) {
            case 'NORD': // Sassari
                initCenter = [40.7259, 8.5566]; break;
            case 'SUD': // Cagliari
    ...
    async loadHospitals() {
        try {
            // Carica SEMPRE tutti gli ospedali di NORD e SUD, indipendentemente dalla centrale selezionata
            // come nel vecchio sistema con le 4 centrali
            let hospitalsAll = [];
            
            // Carica ospedali del NORD
            try {
                ...
                console.warn("Errore caricamento ospedali nord:", e);
            }
            
            // Carica ospedali del SUD
            try {
                ...
                console.warn("Errore caricamento ospedali sud:", e);
            }
            
            // Assegna tutti gli ospedali (NORD + SUD)
            this.hospitals = hospitalsAll;
            console.log(`[INFO] Caricati ${hospitalsAll.length} ospedali totali (NORD + SUD)`);
    ...
    updatePostazioneMarkers() {
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
           // Filtra postazioni per centrale: mostra postazioni esterne con sfondo bianco
           const currentCentral = (window.selectedCentral||'').trim().toUpperCase();
           
           // Determina se la postazione appartiene alla centrale selezionata
           const isPostazioneNord = postazione.isNORD;
           const isPostazioneSud = postazione.isSUD;
           
           // Determina se è una postazione "esterna" (da evidenziare con sfondo bianco)
           let isEsterna = false;
           if (currentCentral === 'NORD' && isPostazioneSud) {
               isEsterna = true; // Mostra postazioni SUD in bianco quando visualizzi NORD
           } else if (currentCentral === 'SUD' && isPostazioneNord) {
               isEsterna = true; // Mostra postazioni NORD in bianco quando visualizzi SUD
           }
            ...
            // Usa sfondo bianco per postazioni esterne alla centrale selezionata
            ...
            marker.on('popupopen', () => {
                // Determine if postazione is external (for icon update)
                const currentCentralNow = (window.selectedCentral||'').trim().toUpperCase();
                const isPostazioneNord = postazione.isNORD;
                const isPostazioneSud = postazione.isSUD;
                
                let isSpecial = false;
                if (currentCentralNow === 'NORD' && isPostazioneSud) {
                    isSpecial = true;
                } else if (currentCentralNow === 'SUD' && isPostazioneNord) {
                    isSpecial = true;
                }
