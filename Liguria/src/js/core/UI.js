...
function getColoreCodice(codice) {
    switch (codice) {
        case 'ROSSO':
        case 'Rosso': return '#f90400ff';
        case 'GIALLO':
        case 'Giallo': return '#eeff00ff';
        case 'VERDE':
        case 'Verde': return '#07f80fff';
        case 'BIANCO':
...
class GameUI {
    ...
    updateStatoMezzi(mezzoCambiato = null) {
        ...
        // Funzione per ottenere l'icona del mezzo
        function getMezzoIcon(tipoMezzo) {
            const tipo = (tipoMezzo || '').toUpperCase();
            switch (tipo) {
                case 'MSB':
                    return 'src/assets/MSB.png';
                case 'MSA1':
                case 'MSA2':
                    return 'src/assets/MSA.png';
                case 'ELI':
                    return 'src/assets/ELI.png';
                default:
                    return 'src/assets/MSB.png'; // Default fallback
            }
        }
        ...
        const mezzi = allMezzi.filter(m => {
            ...
            // Vehicles of selected central - confronto case-insensitive
            if ((m.central || '').trim().toUpperCase() === currentCentral) return true;
        ...
        merged.forEach(m => {
                ...
                const mezzoIcon = getMezzoIcon(m.tipo_mezzo);
                stateDiv.insertAdjacentHTML('beforeend', `
                    <div class="mezzo-row" data-mezzo-id="${m.nome_radio}" style="${rowStyle}">
                        <div class="mezzo-cell" style="flex:3;overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;">
                            <img src="${mezzoIcon}" alt="${m.tipo_mezzo}" style="width:20px;height:20px;margin-right:8px;vertical-align:middle;" />
                            ${displayName}
                        </div>
        ...
        // Dynamic grouping per centrale operativa - Logica per ASL Liguria
        const center = (window.selectedCentral || 'asl 3').toLowerCase();
        ...
        const ospedaliConOccupazione = window.game.hospitals.map(h => {
            ...
            // Determina la centrale di appartenenza dell'ospedale
            let centraleOspedale = 'altro';
            if (h.central) {
                centraleOspedale = h.central.toLowerCase();
            }
            
            return {
                ...h,
                pct: pct,
                centraleOspedale: centraleOspedale
            };
        });
        
        // Separa ospedali della centrale corrente da quelli delle altre centrali
        const ospedaliCentraleCorrente = ospedaliConOccupazione.filter(h => 
            h.centraleOspedale === center
        );
        const ospedaliAltreCentrali = ospedaliConOccupazione.filter(h => 
            h.centraleOspedale !== center
        );
        
        // Ordina per percentuale di occupazione decrescente
        ospedaliCentraleCorrente.sort((a, b) => b.pct - a.pct);
        ospedaliAltreCentrali.sort((a, b) => b.pct - a.pct);
        
        // Unisci: prima la centrale corrente, poi le altre
        const ospedaliOrdinati = [...ospedaliCentraleCorrente, ...ospedaliAltreCentrali];
        
        // Visualizza gli ospedali ordinati con intestazioni di gruppo
        let currentGroup = null;
        ospedaliOrdinati.forEach((h, index) => {
            // Aggiungi intestazione di gruppo quando necessario
            const isOspedaleCentraleCorrente = h.centraleOspedale === center;
            const groupName = isOspedaleCentraleCorrente ? 'Centrale Corrente' : 'Altre Centrali';
            
            if (currentGroup !== groupName) {
                currentGroup = groupName;
                // Aggiungi separatore solo se non è il primo gruppo
                if (index > 0) {
                ...
                hospDiv.insertAdjacentHTML('beforeend', `
                    <div style="padding:4px 4px 2px 4px;font-weight:bold;font-size:12px;color:#555;background:#f5f5f5;">
                        ${groupName}
                    </div>
                `);
            }
            ...
            // Stile diverso per ospedali della centrale corrente vs altre centrali
            const ospedaleStyle = isOspedaleCentraleCorrente ? 
                "padding:2px 4px;border-bottom:1px solid #eee;" : 
                "padding:2px 4px;border-bottom:1px solid #eee;background-color:#f9f9f9;color:#666;";
            
            hospDiv.insertAdjacentHTML('beforeend', `
                <div style="${ospedaleStyle}">
                    ${h.nome} (${pct}%)
                </div>
            `);
        });
