...
function gestisciStato3(mezzo, call) {
    ...
    if (call && call.selectedChiamata) {
        ...
        if (mezzoType.startsWith('MSB')) {
            ...
            // MSA1 usa il proprio report
            reportKey = 'MSA1';
        } else if (mezzoType.startsWith('MSA2')) {
            // MSA2 usa il proprio report
            reportKey = 'MSA2';
            ...
            // MSA generico e VLV usano il report di MSA2
            reportKey = 'MSA2';
...
class EmergencyDispatchGame {
    constructor() {
        ...
        // Counters for mission numbers per C.O. central
        this.missionCounter = { 'asl 1': 0, 'asl 2': 0, 'asl 3': 0, 'asl 4': 0, 'asl 5': 0 };
        ...
        const getRate = () => {
            ...
            let lambdaPeak = 1/60;       // peak: 1 call per 60s
            let lambdaNightPeak = 1/90;  // night peak: 1 call per 90s
            let lambdaDay = 1/120;       // daytime: 1 call per 120s
            let lambdaNight = 1/300;     // night: 1 call per 300s
            
            // Fattori di riduzione per ASL specifiche
            const currentCentral = (window.selectedCentral || '').toLowerCase();
            let reductionFactor = 1.0; // ASL 3 (Genova) mantiene il rate normale
            
            switch(currentCentral) {
                case 'asl 1': // Imperia - riduci di 1/3
                    reductionFactor = 2/3;
                    break;
                case 'asl 2': // Savona - riduci di 1/4
                    reductionFactor = 3/4;
                    break;
                case 'asl 3': // Genova - mantieni normale
                    reductionFactor = 1.0;
                    break;
                case 'asl 4': // Tigullio - dimezza
                    reductionFactor = 1/2;
                    break;
                case 'asl 5': // La Spezia - riduci di 1/3
                    reductionFactor = 2/3;
                    break;
                default:
                    reductionFactor = 1.0;
            ...
            // Applica il fattore di riduzione ai rate
            lambdaPeak *= reductionFactor;
            lambdaNightPeak *= reductionFactor;
            lambdaDay *= reductionFactor;
            lambdaNight *= reductionFactor;
    ...
    async initialize() {
        ...
        const selectedCentral = window.selectedCentral || 'asl 3';
        console.log('[DEBUG initialize] Centrale selezionata:', selectedCentral);
        
        switch (selectedCentral) {
            case 'asl 1':
                window.selectedMezziFile = 'src/data/Mezzi asl 1.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 1.json';
                window.selectedHemsFile = 'src/data/Creli.json';
                break;
            case 'asl 2':
                window.selectedMezziFile = 'src/data/Mezzi asl 2.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 2.json';
                window.selectedHemsFile = 'src/data/Creli.json';
                break;
            case 'asl 3':
                window.selectedMezziFile = 'src/data/Mezzi asl 3.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 3.json';
                window.selectedHemsFile = 'src/data/Creli.json';
                break;
            case 'asl 4':
                window.selectedMezziFile = 'src/data/Mezzi asl 4.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 4.json';
                window.selectedHemsFile = 'src/data/Creli.json';
                break;
            case 'asl 5':
                window.selectedMezziFile = 'src/data/Mezzi asl 5.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 5.json';
                window.selectedHemsFile = 'src/data/Creli.json';
                break;
            default:
                console.warn('Centrale ASL non riconosciuta, usando default asl 3');
                window.selectedMezziFile = 'src/data/Mezzi asl 3.json';
                window.selectedOspedaliFile = 'src/data/Ospedali asl 3.json';
                window.selectedHemsFile = 'src/data/Creli.json';
        }
    ...
    getCentraliLimitrofe(centrale) {
        // Tutte le centrali ASL della Liguria
        const tutteLeASL = ['asl 1', 'asl 2', 'asl 3', 'asl 4', 'asl 5'];
        
        // Restituisce tutte le centrali tranne quella attuale
        return tutteLeASL.filter(asl => asl !== centrale);
    }
    ...
    initializeMap() {
        ...
        let initCenter = [44.0678, 8.5568]; // default Genova (asl 3)
        const initZoom = 11;
        switch (window.selectedCentral) {
            case 'asl 1': // Imperia
                initCenter = [43.8881, 8.0276]; break;
            case 'asl 2': // Savona
                initCenter = [44.3091, 8.4811]; break;
            case 'asl 3': // Genova
                initCenter = [44.4056, 8.9463]; break;
            case 'asl 4': // Tigullio (Lavagna)
                initCenter = [44.3065, 9.3447]; break;
            case 'asl 5': // La Spezia
                initCenter = [44.1025, 9.8246]; break;
        }
    ...
    async loadMezzi() {
        ...
        const currentCentral = window.selectedCentral || 'asl 3';
        const mezziFile = window.selectedMezziFile || 'src/data/Mezzi asl 3.json';
        const hemsFile = window.selectedHemsFile || 'src/data/Creli.json';
        ...
        for (const centrale of centraliLimitrofe) {
            const limitrofaMezziFile = `src/data/Mezzi ${centrale}.json`;
            try {
    ...
    async loadHospitals() {
        try {
            ...
            const ospedaliFile = window.selectedOspedaliFile || 'src/data/Ospedali asl 3.json';
            let hospitalsAll = [];
            ...
            const centraliLimitrofe = this.getCentraliLimitrofe(currentCentral);
            for (const centrale of centraliLimitrofe) {
                const limitrofaOspedaliFile = `src/data/Ospedali ${centrale}.json`;
                try {
    ...
    updatePostazioneMarkers() {
        ...
        Object.values(this.postazioniMap).forEach(postazione => {
           ...
           // Tutte le postazioni di centrali diverse dalla corrente hanno sfondo bianco
           ...
           if (postazioneCentral && postazioneCentral !== currentCentral) {
            ...
            marker.on('popupopen', () => {
                ...
                // Le postazioni esterne devono rimanere bianche, non devono cambiare colore
                const isEsterna = postazioneCentralNow !== currentCentralNow;
                ...
                // Solo le postazioni della centrale corrente possono cambiare colore in base ai mezzi liberi
                marker.setIcon(this.getPostazioneIcon(isEsterna ? false : hasLiberiNow, isEsterna));
            });
    ...
    updateMezzoMarkers() {
        ...
        this.mezzi.forEach(m => {
            ...
            let iconUrl = 'src/assets/MSB.png'; // Default icon for MSB
            if (m.tipo_mezzo && m.tipo_mezzo.toUpperCase().includes('ELI')) {
                iconUrl = 'src/assets/ELI.png';
            } else if (m.tipo_mezzo && (m.tipo_mezzo.startsWith('MSA') || m.tipo_mezzo.startsWith('ILS') || m.tipo_mezzo.startsWith('ALS') || m.tipo_mezzo.startsWith('VLV'))) {
    ...
    generateNewCall() {
        ...
        const unita = year % 10;
        const central = window.selectedCentral || 'asl 3';
        const codeMap = { 'asl 1': 1, 'asl 2': 2, 'asl 3': 3, 'asl 4': 4, 'asl 5': 5 };
        const code = codeMap[central] || 3;
    ...
    openMissionPopup(call) {
        ...
        // Preseleziona i valori se già impostati nella chiamata
        const luogoSelect = document.getElementById('luogo');
        if (luogoSelect && call.luogo) luogoSelect.value = call.luogo;
        
        const patologiaSelect = document.getElementById('patologia');
        if (patologiaSelect && call.patologia) patologiaSelect.value = call.patologia;
        
        const codiceSelect = document.getElementById('codice');
        if (codiceSelect && call.codice) codiceSelect.value = call.codice;
        
        const noteEventoTextarea = document.getElementById('note-evento');
        if (noteEventoTextarea && call.noteEvento) noteEventoTextarea.value = call.noteEvento;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (startButton && centralSelect && overlay) {
        startButton.addEventListener('click', async function() {
            ...
            switch (selectedCentral) {
                case 'asl 1':
                    mezziFile = 'src/data/Mezzi asl 1.json';
                    indirizziFile = 'src/data/Indirizzi asl 1.json';
                    ospedaliFile = 'src/data/Ospedali asl 1.json';
                    hemsFile = 'src/data/Creli.json';
                    document.querySelector('.header-title').textContent = 'Imperia Soccorso';
                    break;
                case 'asl 2':
                    mezziFile = 'src/data/Mezzi asl 2.json';
                    indirizziFile = 'src/data/Indirizzi asl 2.json';
                    ospedaliFile = 'src/data/Ospedali asl 2.json';
                    hemsFile = 'src/data/Creli.json';
                    document.querySelector('.header-title').textContent = 'Savona Soccorso';
                    break;
                case 'asl 3':
                    mezziFile = 'src/data/Mezzi asl 3.json';
                    indirizziFile = 'src/data/Indirizzi asl 3.json';
                    ospedaliFile = 'src/data/Ospedali asl 3.json';
                    hemsFile = 'src/data/Creli.json';
                    document.querySelector('.header-title').textContent = 'Genova Soccorso';
                    break;
                case 'asl 4':
                    mezziFile = 'src/data/Mezzi asl 4.json';
                    indirizziFile = 'src/data/Indirizzi asl 4.json';
                    ospedaliFile = 'src/data/Ospedali asl 4.json';
                    hemsFile = 'src/data/Creli.json';
                    document.querySelector('.header-title').textContent = 'Tigullio Soccorso';
                    break;
                case 'asl 5':
                    mezziFile = 'src/data/Mezzi asl 5.json';
                    indirizziFile = 'src/data/Indirizzi asl 5.json';
                    ospedaliFile = 'src/data/Ospedali asl 5.json';
                    hemsFile = 'src/data/Creli.json';
                    document.querySelector('.header-title').textContent = 'La Spezia Soccorso';
                    break;
                default:
                    console.error('Centrale ASL non riconosciuta:', selectedCentral);
                    return;
            }
